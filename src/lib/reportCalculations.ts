...
export const sectorBenchmarks: Record<string, {
  ...
  fashion: { emailShare: 28, revenuePerSub: 2.0, openRate: 18, clickRate: 2.8, label: 'Abbigliamento & Accessori' },
  ...
  home: { emailShare: 22, revenuePerSub: 1.8, openRate: 19, clickRate: 2.6, label: 'Casa & Arredamento' },
  health: { emailShare: 32, revenuePerSub: 2.8, openRate: 24, clickRate: 3.5, label: 'Salute & Integrazione' },
  sport: { emailShare: 24, revenuePerSub: 2.2, openRate: 20, clickRate: 2.8, label: 'Sport & Outdoor' },
  vino: { emailShare: 26, revenuePerSub: 2.6, openRate: 22, clickRate: 3.0, label: 'Vino & Spirits' },
...
// Direct-values version for admin mode
...
const _calculateReport = (
  ...
  const missingFlows = allFlows
    ...
    .map(key => {
      ...
      return {
        ...
        description: flow.description, implementationTime: flow.implementationTime
      };
    })
  ...
  if (missingFlows.length > 1) {
    const f = missingFlows[1];
    topActions.push({ action: `Attiva ${f.label}`, impact: f.impactValue, difficulty: 'Media', timeframe: f.implementationTime, roi: `+€${Math.round(f.impactValue).toLocaleString()}/mese` });
  } else {
    topActions.push({ action: 'Ottimizza frequenza invio campagne', impact: currentEmailRevenue * 0.1, difficulty: 'Bassa', timeframe: '1 settimana', roi: `+€${Math.round(currentEmailRevenue * 0.1).toLocaleString()}/mese` });
  }
  if (missingFlows.length > 2) {
    const f = missingFlows[2];
    topActions.push({ action: `Configura ${f.label}`, impact: f.impactValue, difficulty: 'Media', timeframe: f.implementationTime, roi: `+€${Math.round(f.impactValue).toLocaleString()}/mese` });
  } else {
    topActions.push({ action: 'Implementa segmentazione RFM avanzata', impact: currentEmailRevenue * 0.08, difficulty: 'Alta', timeframe: '2-3 settimane', roi: `+€${Math.round(currentEmailRevenue * 0.08).toLocaleString()}/mese` });
  }
  ...
  if (automationRating === 'C' || automationRating === 'D') potentialObstacles.push("Competenze tecniche limitate per implementare automazioni avanzate");
  if (activeFlowsCount < 3) potentialObstacles.push("Assenza di strategia strutturata per le automazioni email");
  if (currentEmailPercent < 15) potentialObstacles.push("Lista email sottosviluppata o non adeguatamente segmentata");
  ...
  if (popupParams && popupParams.hasPopup) {
    ...
    popupData = {
      hasPopup: true, conversionRate: popupParams.conversionRate,
      monthlyVisitors: popupParams.monthlyVisitors, newSubscribersPerMonth,
      projectedListSize6m, projectedListSize12m, projectedRevenue12m,
      monthlyListGrowthRate: popupParams.monthlyListGrowthRate,
      revenueWelcome12m, revenueRecovery12m, revenueAutomation12m,
    };
  } else if (popupParams && !popupParams.hasPopup) {
    popupData = {
      hasPopup: false, conversionRate: 0, monthlyVisitors: 0,
      newSubscribersPerMonth: 0, projectedListSize6m: listSize,
      projectedListSize12m: listSize, projectedRevenue12m: 0,
      monthlyListGrowthRate: 0, revenueWelcome12m: 0,
      revenueRecovery12m: 0, revenueAutomation12m: 0,
    };
  }
