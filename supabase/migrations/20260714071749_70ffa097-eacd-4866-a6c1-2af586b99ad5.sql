...
BEGIN
  ...
  -- Only allow finalizing rows still in progress and not already finalized
  IF v_row_report IS NOT NULL OR v_row_status IS DISTINCT FROM 'in_progress' THEN
    RETURN false;
  END IF;

  -- Email in the survey row must match the partial's email
...
GRANT EXECUTE ON FUNCTION public.finalize_submission(uuid, text, text, text, text, text, text, text, text[], text, numeric, numeric, numeric, numeric, numeric, text, jsonb) TO anon, authenticated;
